...
# Generated: Fri Apr 27 01:42:23 2018
##################################################
...
class gfsk_rx(grc_wxgui.top_block_gui):
    ...
    def __init__(self, default_baud=1200, default_bin_file_sink="/tmp/rx_data.bin", default_freq=437.5e6, default_samp=1.8e6, sdr_dev="rtl=0"):
...
def argument_parser():
    ...
    parser.add_option(
        "-b", "--default-baud", dest="default_baud", type="intx", default=1200,
    ...
    parser.add_option(
        "-s", "--default-samp", dest="default_samp", type="eng_float", default=eng_notation.num_to_str(1.8e6),
...
def main(top_block_cls=gfsk_rx, options=None):
    set_procname("gfsk_rx")
    if options is None:
        options, _ = argument_parser().parse_args()
    if gr.enable_realtime_scheduling() != gr.RT_OK:
        print "Error: failed to enable real-time scheduling."

    tb = top_block_cls(default_baud=options.default_baud, default_bin_file_sink=options.default_bin_file_sink, default_freq=options.default_freq, default_samp=options.default_samp, sdr_dev=options.sdr_dev)
    tb.Start(True)
    tb.Wait()
