...
class ReceiveThread(threading.Thread):
    ...
    def run(self):
        while(1):
            input_bits = self.receive_bit()
            # print(input_bits)
            if(len(input_bits)):
                for bit in input_bits:
                    bit_queue.put(bit)

            # print(bit_queue.empty())
    ...
    def receive_bit(self):
        if FROM_FILE:
            ...
            input_bit = file_data[data_counter]
            data_counter += 1
            return int(input_bit)

        input_bits = self.client.receive_data()
...
if __name__ == "__main__":
    ...
    if FROM_FILE:
        ...
        f = open(FILE_PATH, 'rb')
        # file_data = f.read()
        file_data_hex = b'\x7E\x41\x42\x43\x44\x45\x46\xe0\x55\x56\x57\x58\x59\x5a\xe1\x03\xf0\x18\x07\x67\x45\x23\x01\xbb\xaa\x01\x00\x60\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a\x6b\x6c\x6d\x6e\x6f\x93\x4d\x7E'
        ...
        f_tx = open(FILE_PATH_TX, 'wb')
        f_tx.write(file_data)
        f_tx.close()


    # rx_thread = ReceiveThread(name = "ReceiveThread")
    ps_thread = ProcessThread(name = "ProcessThread") 
    # ps_thread.setDaemon(True)
