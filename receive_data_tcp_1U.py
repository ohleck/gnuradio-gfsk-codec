        ...
        while(1):
            if not packet_queue.empty():
                ...
                if ax25_packet.valid:
                    ...
                    if len(ax25_packet.byte_packet) == 274:
                        self.valid_length_packets += 1
                        print("Invalid CRC but right length found: ")
                        print(packet.hex())
                        print("CRC Failed Packet N#: ", self.valid_length_packets)
                        flips = self.count_bit_flips(REF_PACKET, packet)
                        if flips > 20: continue
                        print("N# of bitflips in packet: ", flips)
                        self.bit_flips += flips
                        print("Accumulated N# bitflips: ", self.bit_flips)
                        print()

    def count_bit_flips(self, ref_byte, byte):
        flips = 0
        i = 0
        for b in ref_byte:
            bin_r = b ^ byte[i]
            flips += bin(bin_r).count("1")
            i += 1
